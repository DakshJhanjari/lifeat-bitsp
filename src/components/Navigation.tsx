    ...
    <nav className="fixed top-0 left-0 right-0 bg-white/95 backdrop-blur-lg shadow-lg z-50 border-b-2 border-gradient-to-r from-blue-200 to-purple-200">
      <div className="container mx-auto px-3 sm:px-4">
        <div className="flex items-center justify-between h-14 sm:h-16">
          <div className="flex items-center space-x-1 sm:space-x-2 min-w-0 flex-shrink-0 max-w-[60%] sm:max-w-none">
            <img
          ...
          <div className="hidden md:flex space-x-1 lg:space-x-2 flex-shrink-0">
            {navItems.map((item) => (
              <button
                ...
                className="flex items-center space-x-1 lg:space-x-2 px-2 lg:px-3 py-2 text-slate-700 hover:text-white hover:bg-gradient-to-r hover:from-blue-500 hover:to-purple-500 rounded-lg transition-all duration-300 text-xs lg:text-sm font-semibold shadow-sm whitespace-nowrap"
              >
          ...
          <div className="md:hidden py-3 border-t border-purple-200 bg-gradient-to-r from-blue-50 to-purple-50 -mx-3 sm:-mx-4 px-3 sm:px-4">
            {navItems.map((item) => (
              <button
                key={item.href}
                onClick={() => scrollToSection(item.href)}
                className="flex items-center space-x-3 w-full text-left py-2.5 px-3 text-slate-700 hover:text-white hover:bg-gradient-to-r hover:from-blue-500 hover:to-purple-500 transition-all duration-300 rounded-lg mx-1 font-semibold text-sm"
              >
